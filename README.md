...
它适合地推餐饮商家时使用：你提供菜品原图、店名、菜名、价格、平台、场景关键词等信息，skill 会先根据关键词锁定任务类型，再问你是否还要补充几个关键点；你确认后，它会匹配内置的原始中文餐饮宣传图提示词模板，只替换模板里的 `【】` 占位符，不私自改写提示词，最后调用 image2.0 / image generation 生成可直接发美团、大众点评、朋友圈、抖音本地生活或小红书的宣传图。
...
3. 出图前列出 3-6 个可选补充点，比如平台比例、必须放的文字、卖点、风格、不能改的地方、是否统一风格。
4. 用户确认补充或说“没有/直接做”后，从 `references/pdf-prompt-index.md` 中匹配合适的原始中文餐饮提示词模板。
5. 只替换模板里的 `【店名】`、`【菜品名】`、`【价格】` 等占位符；如果无法判断场景，就使用“通用海报提示词”。
6. 生成图片，并简短说明匹配到的任务类型和模板。
